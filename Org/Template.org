...
#+end_src
** TODO Update Script
This code is a script file to update the Build tools in a current project with
the updated Build tools from this template.  It copies the outline structure of
the Build Tools from this template file into the current project’s Org file.
...
#+begin_src emacs-lisp
  ...
  an Org file with a current Build Tools subtree."
    ...
    (require (quote org))
    (save-excursion 
      (find-file "~/Dev/Templates2/Org/Template.org")
      (goto-char (point-min))
