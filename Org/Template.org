...
#+begin_src sh
  [[ -z ${AWSS_BUCKET} ]] && { printf "${RED}ERROR: ${CYAN}\$AWS_BUCKET must be set${CLEAR}"; exit 0; }
...
#+begin_src sh
  # $1 := Title
  [[ ($# -eq 1) || ($# -eq 2) ]] || {
      printf "${RED}ERROR: \'org-tempate <TITLE> [<AUTHOR>]\'${CLEAR}"
      exit 1
  }
  read -N 1 -p "Create new directory \'$1\'?"
  [[ $REPLY =~ [yY] ]] && echo || { printf "${RED}abort${CLEAR}"; exit 0; }
